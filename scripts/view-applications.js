...
function listApplications() {
  ...
  files.forEach((file, index) => {
    try {
      ...
      console.log(
        `   🏠 Room: ${data.hallName || 'Unknown'} - ${data.roomNumber || 'N/A'}`
      );
...
function updateStatus(fileName, newStatus) {
  ...
  try {
    ...
    console.log(
      `✅ Status updated to ${newStatus.toUpperCase()} for ${data.fullName || 'Anonymous'}`
    );
  } catch (error) {
...
switch (command) {
  ...
  case 'show':
    if (!arg) {
      console.log(
        '❌ Please provide a filename. Usage: node view-applications.js view <filename>'
      );
      process.exit(1);
    }
    viewApplication(arg);
  ...
  case 'approve':
    if (!arg) {
      console.log(
        '❌ Please provide a filename. Usage: node view-applications.js approve <filename>'
      );
      process.exit(1);
    }
  ...
  case 'reject':
    if (!arg) {
      console.log(
        '❌ Please provide a filename. Usage: node view-applications.js reject <filename>'
      );
      process.exit(1);
    }
  ...
  case 'rm':
    if (!arg) {
      console.log(
        '❌ Please provide a filename. Usage: node view-applications.js delete <filename>'
      );
      process.exit(1);
    }
  ...
  default:
    ...
    console.log('Usage:');
    console.log(
      '  node view-applications.js list                    - List all applications'
    );
    console.log(
      '  node view-applications.js view <filename>         - View specific application'
    );
    console.log(
      '  node view-applications.js approve <filename>      - Approve application'
    );
    console.log(
      '  node view-applications.js reject <filename>       - Reject application'
    );
    console.log(
      '  node view-applications.js delete <filename>       - Delete application'
    );
    console.log('\nExamples:');
    console.log('  node view-applications.js list');
    console.log(
      '  node view-applications.js view application_123_1234567890.json'
    );
    console.log(
      '  node view-applications.js approve application_123_1234567890.json'
    );
}
